""" removed unique constraint from vualt model2

Revision ID: 4f6fdef2067e
Revises: 3592d7cd58ee
Create Date: 2025-09-14 12:56:31.393388
...
revision: str = '4f6fdef2067e'
down_revision: Union[str, Sequence[str], None] = '3592d7cd58ee'
...
def downgrade() -> None:
    ...
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('now()'), autoincrement=False, nullable=False),
    sa.Column('updated_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('now()'), autoincrement=False, nullable=False),
    sa.ForeignKeyConstraint(['owner_id'], ['user.id'], name=op.f('vault_owner_id_fkey')),
    sa.PrimaryKeyConstraint('id', name=op.f('vault_pkey'))
    )
